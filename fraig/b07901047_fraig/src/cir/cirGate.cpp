...
size_t
GateV::simVal() const {
    return (isInv() ? (~(gate()->_simValue)) : (gate()->_simValue));
}
/**************************************/
...
CirGate::reportGate() const
{
    ...
    cout << "================================================================================ " << endl 
         << "= " <<       gInfo    << endl 
         << "= FECs: ";
    if((isAig()||!_id) && !(((AIGate*)this)->_fecId==(unsigned)(-1))){
        bool phase = 0; 
        auto grp = cirMgr->_fecGrps[ ((AIGate*)this)->_fecId ];
        unsigned i = grp.size()-1, cnt = 0;
        for(auto lit: grp)
            if(lit/2==_id) { phase = lit%2; break;}
        for(auto lit: grp){
            if(lit/2==_id) continue;
            if((lit%2+phase)%2==1) cout << "!";
            cout << (lit/2);
            if(cnt!=i-1) cout << " ";
            ++cnt;
        }
    ...
    cout << endl;
    cout << "= Value: ";
    for(unsigned i=1; i<PATNUM+1; ++i){
        cout << (_simValue>>(i-1))%2;
        if(!(i%8) && i!=PATNUM) cout << "_";
    }
    cout << endl;
    cout << "================================================================================ " << endl; 
}
