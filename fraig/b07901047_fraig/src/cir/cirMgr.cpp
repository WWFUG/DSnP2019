...
CirMgr::getGate( unsigned gid ) const {
    if( gid < 0 || gid > _miloa[0]+_miloa[3] || !_gateList[gid] )
        return 0;
...
CirMgr::printFECPairs() const
{
    unsigned cnt = 0;
    bool phase = 0;
    sort(_fecGrps.begin(), _fecGrps.end(), [](const IdList& a, const IdList& b){return a[0]<b[0];} );
    for(auto& grp: _fecGrps){
        phase = (grp[0]%2==1);
        cout << "[" << cnt << "] ";
        for(auto it = grp.begin(); it!=grp.end(); ++it){
            _gateList[(*it)/2]->setFecId(cnt);
            if(phase+(*it)%2==1) cout << "!";
            cout << (*it)/2;
            if(it!=grp.end()-1) cout << " ";
        }
        cout << endl; ++cnt;
    }
}
